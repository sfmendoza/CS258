...
void setup() {
  ...
  output = createWriter("HeartRate.txt");
...
void draw() {
  ...
  zoom = 0.5;                                    // get current waveform scale value
  ...
  bezier(width-100, 50, width-20,  -20, width,     140, width-100, 160);
  bezier(width-100, 50, width-190, -20, width-200, 140, width-100, 160);
  strokeWeight(1);          // reset the strokeWeight for next time


  // PRINT THE DATA AND VARIABLE VALUES
  fill(eggshell);                                       // get ready to print text
  text("CS 258 Heart Rate Sensor",245,30);              // tell them what you are
  text("Beat Interval " + IBI + "mS",550,585);                    // print the time between heartbeats in mS
